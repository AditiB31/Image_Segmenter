...
- **app.py** — Flask routes and session management. Each upload gets a UUID-based session. Sessions auto-cleanup after `session_ttl` seconds. Image routes: `/upload` (POST), `/segment/<session_id>` (POST), `/segment-image/`, `/download/`, `/download-all/`. Manual annotation routes: `/original-image/<session_id>` (GET), `/annotate/<session_id>` (POST). PDF routes: `/upload-pdf` (POST), `/slide-image/`, `/segment-slide/`. Browse routes: `/browse/runs`, `/browse/run/<name>`, `/browse/run/<name>/<slide>`, `/browse/thumb/`, `/browse/download/`.
- **segmenter.py** — `ImageSegmenter` class wrapping SAM 2.1's `SAM2AutomaticMaskGenerator` and `SAM2ImagePredictor`. The `segment()` method does automatic segmentation; the `predict_with_prompts()` method accepts user-provided point, polygon, and box prompts for manual annotation. Both share the same SAM2 model weights. Handles device selection (MPS -> CPU fallback), image resizing for inference, mask filtering/sorting, and saving cropped RGBA segments with edge feathering and tight cropping.
...
- **templates/index.html** + **static/js/app.js** + **static/css/style.css** — Single-page frontend with three-mode UI: Upload Image, Upload PDF, Browse Runs. After image upload, users choose between Auto Segment All and Manual Annotate. Manual mode provides a canvas with point, polygon, and box annotation tools; extracted segments appear immediately in a gallery strip with per-segment removal and ZIP download. Supports slide navigation, segment gallery with selection/filtering/sorting, and modal preview.
