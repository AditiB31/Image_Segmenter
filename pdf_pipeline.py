...
Folder structure (default, PDF inside data/pdfs/):
    data/images/<pdfname>_<datetime>/slide_001.png, ...
    data/segments/<pdfname>_slide_001_<datetime>/segment_000.png, ...

Folder structure (external PDF):
    <pdf_parent>/slide_images/<pdfname>_<datetime>/slide_001.png, ...
    <pdf_parent>/slide_image_segments/<pdfname>_slide_001_<datetime>/segment_000.png, ...
"""
...
def pdf_to_images(pdf_path, output_dir, dpi=200, fmt="png", page_indices=None):
    ...
    Returns list of (page_number, path) tuples for rendered pages.
    """
    import fitz  # PyMuPDF
...
Examples:
  ...
  python pdf_pipeline.py deck.pdf --slides 1-5 --upscale 2 --min-area 200
    ...
    images_dir = os.path.join(images_base, f"{pdf_name}_{timestamp}")
    ...
    # Resolve slide selection before rasterizing so we only render needed pages
    page_indices = None
    ...
    print(f"  Segments:     {segments_base}/")
    print(f"{'=' * 60}\n")
    ...
    summary_path = os.path.join(images_dir, "pipeline_summary.json")
